...
[0, 1].forEach(i => {
  ...
  input[i].split(',').forEach(l => {
    const dir = l.substr(0, 1);
    const len = parseInt(l.substr(1));
    if (['R', 'L'].includes(dir)) {
      lines[i + 1]['horizontal'].push({ y, x1: x, x2: dir === 'R' ? x + len : x - len, distance });
      x = dir === 'R' ? x + len : x - len;
      distance += len;
    } else {
      lines[i + 1]['vertical'].push({ x, y1: y, y2: dir === 'U' ? y + len : y - len, distance });
      y = dir === 'U' ? y + len : y - len;
      distance += len;
    }
  });
...
const isBetween = (a1, a2, b) => (a1 > b && b > a2) || (a2 > b && b > a1);

const getIntersection = (vertical, horizontal) => {
  if (isBetween(horizontal.x1, horizontal.x2, vertical.x) && isBetween(vertical.y1, vertical.y2, horizontal.y)) {
    // return { x:  vertical.x, y: horizontal.y };
    // return Math.abs(vertical.x) + Math.abs(horizontal.y);
    return vertical.distance + Math.abs(vertical.y1 - horizontal.y) + horizontal.distance + Math.abs(horizontal.x1 - vertical.x);
  }
...
lines[1]['horizontal'].forEach(horizontal => {
  lines[2]['vertical'].forEach(vertical => {
    min = Math.min(min, getIntersection(vertical, horizontal) || Number.MAX_VALUE);
  });
});

lines[2]['horizontal'].forEach(horizontal => {
  lines[1]['vertical'].forEach(vertical => {
    min = Math.min(min, getIntersection(vertical, horizontal) || Number.MAX_VALUE);
  });
});
